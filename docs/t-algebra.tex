...
\end{align}
So we have (\textbackslash2\label{eq:6-2})
\[
...
Now \((a)\) is seen to be equivalent to (@\label{eq:6-2}) upon evaluation.
This proves Claim 2.
...
Now apply same argument as for (@\label{eq:6-1}), (@\label{eq:6-2}) of Theorem @(thm:hd2-module1) and observe \(A^*\) has \(D+1\) distinct eigenvalues. So,
...
To find eigenvalues of \(\tilde{A}\), pick any irreducible \(T(x)\)-module \(W\) with the endpoint \(r\leq D\). Then by Theorem \ref{thm:hd2-module}
\[\text{diam}(W) = N-2r+1.\]
Let \(w_0, w_1, \ldots, w_{N-2r}\) denote a basis for \(W\) as in Theorem \ref{thm:hd2-module}. Then,
...
in this sum by Theorem \ref{thm:hd2-module} \((iv)\).
\end{proof}
...
\hypertarget{lec9}{%
\chapter{\texorpdfstring{Thin \(T\)-Module}{Thin T-Module}}\label{lec9}}
