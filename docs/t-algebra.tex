...
\(\quad\) 4a. The isomorphism class of \(W\) is determined by the intersection numbers and \(a_(W)\)

\(\quad\) 4b. \(\mathrm{Span}(\{v_1^+, v_2^+, \ldots, v^+_D\})\) is thin irreducible \(T(x)\)-module if and only if \(v^+_i, v^-_i\) are dependent, for all \(i\)

\(\quad\) 4c. Id \(m_1 < k_1\), there exist at least one thin, irreducible \(T(x)\)-module with endpoint \(1\)
...
\tightlist
\item
  Distance-regular graphs were each irreducible \(T(x)\)-module with endpoint \(1\) is thin
\end{enumerate}
...
\(\quad\) 5c. \(a_0 = 0\) implies \(a_i = 0\) \((1\leq i\leq D-1)\)
...
\(\quad\) 5g. What minimal assumption (weker than \emph{Q}) implies \emph{Z} (?)
...
\begin{definition}
\protect\hypertarget{def:path}{}\label{def:path}A path\index{path} of length \(\ell\) connecting \(x, y\) is a sequence
\[x = x_0, x_1, \ldots, x_{\ell} = y, \quad x_i\in X, \; 0\leq i\leq \ell\]
such that \(x_ix_{i+1}\in E\) for \(0\leq i \leq \ell-1\).
\end{definition}

\begin{definition}
\protect\hypertarget{def:distance}{}\label{def:distance}The distance\index{distance} \(\partial(x,y)\) is the length of a shortest path connecting \(x\) and \(y\).
\[\partial(x,y) \in \mathbb{Z}^{\geq 0} \cup \{\infty\}.\]
\end{definition}

\begin{definition}
\protect\hypertarget{def:connected}{}\label{def:connected}The graph \(\Gamma\) is connected\index{connected} if and only if \(\partial(x,y) < \infty\) for all \(x, y\in X\).
\end{definition}
...
\begin{definition}
\protect\hypertarget{def:diameter}{}\label{def:diameter}\index{diameter}
...
\begin{definition}
\protect\hypertarget{def:module}{}\label{def:module}A \(T\)-module \index{module} is any subspace \(W\subset V\) such that \(aw\in W\) for all \(a\in T\) and \(w\in W\).
...
\begin{definition}
\protect\hypertarget{def:bipartite}{}\label{def:bipartite}\(\Gamma\) is said to be \emph{bipartite}\index{bipartite} if the adjacency matrix \(A\) is bipartite. That is, \(X\) can be written as a disjoint union of \(X^+\) and \(X^-\) such that \(X^+, X^-\) contain no edges of \(\Gamma\).
\end{definition}
...
\emph{Q}: \(\Gamma\) is \(!\)-polynomial with respect to the ordering \(E_0, E_1, \ldots, E_D\) of the primitive idempotents.
...
\emph{Few2}: The subgraph induced on the second subconstituent of \(\Gamma\) with respect to \(x\) has at most \(16\) distince eigenvalues.
...
\tightlist
\item
  copy: Code \textgreater{} Clone \textgreater{} SSH
\item
