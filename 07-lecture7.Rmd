...
library(tidyverse)
df_eg41 <- data.frame(x = c(0, 2.2, 5, 2.2, 2, 2.2, 3, 2.2, 0, 2, 5, 3, 0), y = c(0, -3, 0, 3, -1, -3, 1, 3, 0, -1, 0, 1, 0))
ggplot(df_eg41, aes(x, y)) + geom_point() + geom_path() + theme_bw() + theme(panel.border = element_blank(),line = element_blank(),
        text = element_blank(),
        title = element_blank()) +
  annotate("text", x=-0.3, y = 0.05, label = "12") + annotate("text", x=1.8, y = -3, label = "14") +
  annotate("text", x=1.7, y = -1.2, label = "13") + annotate("text", x=5.3, y = 0, label = "34") +
  annotate("text", x=1.8, y = 3, label = "23") + annotate("text", x=2.6, y = 1.2, label = "24") 
```
:::
